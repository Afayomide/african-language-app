...
import { Language, Lesson, Level, Status, Word } from "@/types";
...
function WordsByLanguageContent({ params }: { params: Promise<{ language: string }> }) {
  ...
  async function handleBulkFinish() {
    ...
    try {
      ...
      toast.success(`Finished ${finishable.length} word(s)`);
      setSelectedIds([]);
      void fetchItems();
    } catch {
  ...
  async function handleBulkPublish() {
    ...
    if (publishable.length === 0) {
      toast.error("No selected finished words to publish");
      return;
    }
    try {
      await Promise.all(publishable.map((id) => wordService.publishWord(id)));
      toast.success(`Published ${publishable.length} word(s)`);
      setSelectedIds([]);
      void fetchItems();
    } catch {
      toast.error("Failed to bulk publish words");
    }
  }
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="rounded-3xl border-2">
              <DialogHeader>
                <DialogTitle className="text-2xl font-bold">Generate Words with AI</DialogTitle>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="wordGenerationLevel" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Level</Label>
                  <Select value={generationLevel} onValueChange={(value) => setGenerationLevel(value as Level)} disabled={selectedLessonId !== "all"}>
                    <SelectTrigger id="wordGenerationLevel" className="h-12 rounded-xl border-2">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                `/words/new?language=${language}${selectedLessonId !== "all" ? `&lessonId=${selectedLessonId}` : ""}`
              )
            }
          >
