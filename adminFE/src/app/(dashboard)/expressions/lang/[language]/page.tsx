...
function ExpressionsByLanguageContent({
  ...
  async function handleBulkPublishExpressions() {
    ...
    try {
      ...
      toast.error("Failed to bulk publish expressions");
    }
  }

  async function handlePublish(id: string) {
    try {
      await expressionService.publishExpression(id);
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            variant="outline"
            onClick={handleBulkPublishExpressions}
            disabled={selectedExpressionIds.length === 0}
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="rounded-3xl border-2">
              <DialogHeader>
                <DialogTitle className="text-2xl font-bold">Generate Expressions with AI</DialogTitle>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="expressionGenerationLevel" className="font-bold text-xs uppercase tracking-wider text-muted-foreground">Level</Label>
                  <Select
                    value={generationLevel}
                    onValueChange={(value) => setGenerationLevel(value as Level)}
                    disabled={selectedLessonId !== "all"}
                  >
                    <SelectTrigger id="expressionGenerationLevel" className="h-12 border-2 rounded-xl">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                `/expressions/new?language=${language}${
...
export default function ExpressionsByLanguagePage({
  params
}: {
  params: Promise<{ language: string }>;
}) {
