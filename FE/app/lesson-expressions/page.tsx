...
import { LearningContent, Lesson } from '@/types'
...
function LessonExpressionsContent() {
  ...
  const [contentItems, setContentItems] = useState<LearningContent[]>([])
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const [lessonData, expressionsData] = await Promise.all([
          learnerLessonService.getLessonOverview(finalLessonId),
          learnerLessonService.getLessonExpressions(finalLessonId)
        ])
        
        setLesson(lessonData.lesson)
        setContentItems(expressionsData.expressions || [])
      } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4">
              {contentItems.map((item, index) => (
                <Card key={item._id || index} className="p-6 border-4 border-muted rounded-[2rem] bg-white shadow-sm hover:shadow-md transition-all group">
                  <div className="flex items-center justify-between gap-6">
                    <div className="flex-1 space-y-2">
                      ...
                      {item.pronunciation && (
                        <p className="text-sm font-medium text-foreground/30 italic">[{item.pronunciation}]</p>
                      )}
                    </div>
