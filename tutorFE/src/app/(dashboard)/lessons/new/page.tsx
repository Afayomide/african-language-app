...
import { lessonService, authService, aiService, chapterService, unitService } from "@/services";
...
export default function NewLessonPage() {
  ...
  useEffect(() => {
    const loadUnits = async () => {
      try {
        const data = await unitService.listUnits({ chapterId: chapterId || undefined });
        setUnits(data);
        if (initialUnitId) {
    ...
    void loadUnits();
  }, [chapterId, initialUnitId, unitId]);

  const routerLanguage = authService.getTutorProfile()?.language || "yoruba";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const topics = topicsInput.split(",").map((item) => item.trim()).filter(Boolean);
      if (!chapterId) {
  ...
  const handleAISuggest = async () => {
    ...
    try {
      ...
      const suggestion = await aiService.suggestLesson(title, routerLanguage, level);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Place lessons inside the correct chapter and unit instead of creating them flat.</p>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                ...
                <Input value={String(routerLanguage).toUpperCase()} disabled />
              </div>
