...
export default function TutorQuestionsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!isMatchingQuestion && !formData.sourceId) return toast.error("Select an expression")
    if (isMatchingQuestion && formData.matchingItems.length < 2) return toast.error("Select at least two expressions")
    ...
    try {
      ...
      if (isContextScenarioQuestion) {
        if (!options || options.length > 4) {
          return toast.error("Context-response questions must have between 2 and 4 options")
        }
        ...
        if (!selectedExpressionText || correctOption !== selectedExpressionText) {
          return toast.error("For context-response questions, the correct option must exactly match the selected expression text")
        }
      }
      ...
      if (isMatchingQuestion && formData.subtype === "mt-match-image") {
        ...
        if (missingImages) {
          return toast.error("Each selected expression needs at least one linked image")
        }
      }
                ...
                >
                  ...
                  <SelectContent className="rounded-2xl border-2 shadow-xl">
                    {(expressions.find((expression) => expression._id === formData.sourceId)?.translations || []).map((item, index) => (
                      <SelectItem key={`tutor-q-translation-${index}`} value={String(index)} className="py-2">
                        Index {index}: {item}
                      </SelectItem>
                    ))}
                  </SelectContent>
