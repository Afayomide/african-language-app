...
import { ArrowRight, Check, Info, Quote, Turtle, Volume2, X } from 'lucide-react'
import type {
  ExerciseQuestion,
  Language,
...
export function LessonPlayer({
  ...
  const resetStageState = useCallback(() => {
    ...
    resetAnswerState()
  }, [clearStageAdvanceTimeout, resetAnswerState])
            ...
            <div className={cx('flex flex-wrap gap-2', isShortViewport ? 'mb-4' : 'mb-6')}>
              {stageMeta.map((stage) => (
                <button
                  ...
                  onClick={() => handleJumpToStage(stage.index)}
                >
                  Stage {stage.index + 1}
                </button>
              ))}
