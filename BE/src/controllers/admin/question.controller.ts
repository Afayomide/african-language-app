...
export async function updateQuestion(req: Request, res: Response) {
  ...
  if (subtypeUsesMatching(effectiveSubtype)) {
    ...
    const spellingReview = buildLetterOrderReviewData({
      ...
      meaning: getSelectedTranslation(targetSource.translations, Number(update.translationIndex ?? currentQuestion.translationIndex))
    });
    if (!spellingReview) {
      return res.status(400).json({ error: "This spelling question requires a content item with at least two letters." });
    }
  ...
  return res.status(200).json({ question: await serializeQuestionWithSource(question) });
}
