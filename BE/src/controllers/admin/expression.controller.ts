...
import { AdminExpressionUseCases } from "../../application/use-cases/admin/expression/AdminExpressionUseCases.js";
...
const expressionUseCases = new AdminExpressionUseCases(
  lessonRepo,
  expressionRepo,
  lessonContentRepo,
...
async function hydrateExpressionPayloads<T extends { _id?: mongoose.Types.ObjectId | string; id?: string }>(expressions: T[]) {
  ...
  return expressions.map((expression) => {
    const id = String(expression.id || expression._id || "");
    return {
      ...expression,
      lessonIds: lessonIdsByExpression.get(id) || []
    };
  });
}
...
function buildCreateOrUpdateInput(body: Record<string, unknown>, idForAudio?: string) {
  ...
  if (body.components !== undefined) {
    update.components = Array.isArray(body.components) ? body.components as ExpressionEntity["components"] : [];
  }
...
export async function createExpression(req: AuthRequest, res: Response) {
  ...
  if (!baseInput.translations || baseInput.translations.length === 0) {
  ...
  } as ExpressionEntity & { lessonIds?: string[]; createdBy: string });

  if (created === "cannot_add_draft_to_published_lesson") {
    return res.status(400).json({ error: "cannot add draft to published lesson" });
  }
  if (!created) return res.status(400).json({ error: "failed to create expression" });
...
export async function listExpressions(req: AuthRequest, res: Response) {
  ...
  const expressions = await ExpressionModel.find(query)
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(paginationInput.limit)
    .lean();
...
export async function getExpressionById(req: AuthRequest, res: Response) {
  ...
  const expression = await expressionRepo.findById(id);
...
export async function updateExpression(req: AuthRequest, res: Response) {
  ...
  const baseInput = buildCreateOrUpdateInput(req.body ?? {}, id);
  if (baseInput === "invalid_lesson_ids") return res.status(400).json({ error: "invalid lesson id" });
  if (baseInput === "invalid_translations") return res.status(400).json({ error: "at least one translation required" });
  if (baseInput === "invalid_difficulty") return res.status(400).json({ error: "invalid difficulty" });
  if (baseInput === "invalid_register") return res.status(400).json({ error: "invalid register" });
  ...
  const updated = await expressionUseCases.update(id, {
    ...baseInput,
    createdBy: String(req.body?.createdBy || req.user?.id || "system")
  });
  ...
  if (updated === "cannot_add_draft_to_published_lesson") {
    return res.status(400).json({ error: "cannot add draft to published lesson" });
  }
...
export async function deleteExpression(req: AuthRequest, res: Response) {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ error: "invalid id" });
  const deleted = await expressionUseCases.delete(id);
...
export async function bulkDeleteExpressions(req: AuthRequest, res: Response) {
  ...
  const deleted = await expressionUseCases.bulkDelete(ids.map(String));
  return res.status(200).json({ deletedCount: deleted.length, deletedIds: deleted.map((item) => item.id) });
}

export async function publishExpression(req: AuthRequest, res: Response) {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ error: "invalid id" });
  const expression = await expressionUseCases.publish(id);
  if (!expression) return res.status(404).json({ error: "expression not found or not finished" });
  const [payload] = await hydrateExpressionPayloads([expression]);
  return res.status(200).json({ expression: payload });
}
...
export async function generateExpressionAudioById(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    console.error("Admin generateExpressionAudioById TTS error", error);
...
export async function listExpressionImages(req: Request, res: Response) {
  const { id } = req.params;
  ...
  const images = await expressionImageService.listByExpressionId(id);
  return res.status(200).json({
    expressionId: id,
