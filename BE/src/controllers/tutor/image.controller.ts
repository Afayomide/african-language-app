...
export async function createImageAsset(req: AuthRequest, res: Response) {
  ...
  const { url, thumbnailUrl, mimeType, width, height, description, altText, tags, languageNeutralLabel, imageUpload } = req.body ?? {};
...
export async function getImageAssetById(req: AuthRequest, res: Response) {
  ...
  if (image.status !== "approved" && image.uploadedBy !== req.user.id) {
    return res.status(404).json({ error: "image not found" });
  }
...
export async function updateImageAsset(req: AuthRequest, res: Response) {
  ...
  const current = await imageRepo.findById(id);
  if (!current || current.uploadedBy !== req.user.id) {
    return res.status(404).json({ error: "image not found" });
  }
